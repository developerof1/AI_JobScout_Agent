"""HTML/Selenium scrape method — finds heading tags (<h3>/<h2>/<h4>) inside <a> tags.
Used for firms whose job listings render into the page's static or JS-rendered HTML.
"""
...
def _scrape_with_selenium_clicks(start_url: str, keywords: list[str], max_pages: int, skip_keyword_filter: bool = False) -> list[dict]:
    ...
    try:
        ...
        for page_num in range(1, max_pages + 1):
            ...
            new_jobs = [j for j in page_jobs if job_key(j) not in seen_keys]
            for j in new_jobs:
    ...
    for tag_name in ("h3", "h2", "h4", "h1"):
        for heading in soup.find_all(tag_name):
            ...
            if re.search(r'^(page\s+\d+\s*-\s*)?\d+\s+.+\bjobs?\b', title.lower()):
                continue
            if re.search(r'^top companies\b.*\bhiring\b', title.lower()):
                continue
            if not skip_keyword_filter and not title_matches_keywords(title, keywords):
                continue
        ...
        if use_js:
            ...
            while current_url and page_num <= max_pages:
                ...
                for job in _extract_jobs(html, base_url, keywords, skip_kw):
                    if job_key(job) not in seen_keys:
                        url_jobs.append(job)
